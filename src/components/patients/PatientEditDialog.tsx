...
import { validateCPF, validatePhone, formatCPF, formatPhone, normalizePhoneForStorage } from "@/lib/validations";
...
import { InactivationReasonDialog } from "@/components/crm/InactivationReasonDialog";
import { getReasonLabel, REFERRAL_SOURCES } from "@/lib/crm";
  ...
  useEffect(() => {
    if (patient && open) {
      form.reset({
        ...
        referral_detail: patient.referral_detail || "",
        emergency_contact: patient.emergency_contact || "",
        registration_date: patient.registration_date || new Date().toISOString().split("T")[0],
        guardian_name: patient.guardian_name || "",
        guardian_cpf: patient.guardian_cpf ? formatCPF(patient.guardian_cpf) : "",
        guardian_phone: patient.guardian_phone ? formatPhone(patient.guardian_phone) : "",
        guardian_relationship: patient.guardian_relationship || "",
      });
    }
  }, [patient, open, form]);

  const persist = async (data: PatientFormData, reason?: string | null, reasonOther?: string | null) => {
    if (!patient) return;
    setIsLoading(true);
    try {
      const isBecomingInactive = patient.is_active && !data.is_active;
      const isBecomingActive = !patient.is_active && data.is_active;
      const update: any = {
        name: data.name,
      ...
      if (isBecomingInactive) {
        update.inactivation_reason = reason ?? "outros";
        update.inactivation_reason_other = reason === "outros" ? reasonOther : null;
        update.inactivated_at = new Date().toISOString();
      } else if (isBecomingActive) {
        update.inactivation_reason = null;
        update.inactivation_reason_other = null;
        update.inactivated_at = null;
      }

      const { error } = await supabase.from("patients").update(update).eq("id", patient.id);
      if (error) throw error;

      toast({ title: "Paciente atualizado!", description: `${data.name} foi atualizado com sucesso.` });
      onOpenChange(false);
      onSuccess?.();
    } catch (error: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex items-center justify-between rounded-lg border p-3">
                  <div>
                    <FormLabel className="text-base">Paciente Ativo</FormLabel>
                    <p className="text-sm text-muted-foreground">
                      Pacientes inativos não aparecem nas buscas
                    </p>
                  </div>
                  <FormControl>
      ...
      <InactivationReasonDialog
        ...
        onOpenChange={(o) => { setReasonDialogOpen(o); if (!o) setPendingData(null); }}
        initialReason={patient.inactivation_reason ?? null}
        initialOther={patient.inactivation_reason_other ?? null}
        onConfirm={async (reason, other) => {
          setReasonDialogOpen(false);
          if (pendingData) {
            await persist(pendingData, reason, other);
            setPendingData(null);
          }
        }}
      />
